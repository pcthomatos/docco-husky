...
(function() {
  var _, arrayGroupBy, check_config, cloc, content_template, destination, docco_styles, docco_template, dox, dox_template, ensure_directory, exec, ext, file_exists, fs, generate_content, generate_documentation, generate_readme, generate_source_html, get_language, gravatar, highlight, highlight_end, highlight_start, jade, l, languages, ncp, parse, parse_args, parse_markdown, path, ref, relative_base, showdown, spawn, walk, write_file;
  ...
  arrayGroupBy = function(array, aggregate) {
    return array.reduce(function(previous, current, index, context) {
      var group;
  ...
  parse_args(function(sources, project_name, raw_paths) {
    ...
    sources = arrayGroupBy(sources, function(source) {
      console.log("START");
      console.log(source);
      console.log(path.basename(source)[0]);
      console.log("END");
      return path.basename(source)[0];
    });
    context = {
      sources: sources,
      options: {
